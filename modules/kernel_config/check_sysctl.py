...
def scan_sysctl():
    ...
    try:
        ...
        logging.info("✅ sysctl -a output retrieved successfully")
        ...
        report.append(f"❌ Error retrieving sysctl values: {e}")
        logging.error(f"❌ Error retrieving sysctl values: {e}")
        return report
    ...
    try:
        ...
        logging.info("✅ /etc/sysctl.conf loaded successfully")

    except FileNotFoundError:
        report.append("❌ /etc/sysctl.conf not found")
        logging.error("❌ /etc/sysctl.conf not found")
        sysctl_conf_lines = []
    ...
    for rule, expected_value in SYSCTL_HARDENING_RULES.items():
        if rule in sysctl_output:
            ...
            if actual_value == expected_value:
                ...
                logging.info(f"✅ {rule} = {actual_value} (kernel) — OK")
            else:
                report.append(f"❌ {rule} = {actual_value} (kernel), expected {expected_value}")
                logging.warning(f"❌ {rule} = {actual_value} (kernel), expected {expected_value}")
        else:
            report.append(f"⚠️ {rule} not found in sysctl -a — checking sysctl.conf...")
            logging.warning(f"⚠️ {rule} not found in sysctl -a — checking sysctl.conf...")
        ...
        for line in sysctl_conf_lines:
            if rule in line:
                ...
                if line.strip().startswith("#"):
                    ...
                    logging.warning(f"⚠️ {rule} is commented in sysctl.conf — should be uncommented")
                elif f"{rule}={expected_value}" not in line.replace(" ", ""):
                    report.append(f"❌ {rule} found in sysctl.conf but has wrong value")
                    logging.warning(f"❌ {rule} found in sysctl.conf but has wrong value")
                else:
                    report.append(f"✅ {rule} is correctly configured in sysctl.conf")
                    logging.info(f"✅ {rule} is correctly configured in sysctl.conf")
