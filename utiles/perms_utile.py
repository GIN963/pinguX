...
def check_file_perms(path, expected_perms, success_msg, error_msg, report):
    """
    Checks the permissions of a file and appends the result to the report.

    Parameters:
    - path (str): Full path to the file to check.
    - expected_perms (str): The permission string to expect (e.g., "600").
    - success_msg (str): Message template if the permission is correct. Use {path}.
    - error_msg (str): Message template if the permission is incorrect. Use {path}, {expected}, {actual}.
    - report (list): The list that accumulates audit messages.
    """
